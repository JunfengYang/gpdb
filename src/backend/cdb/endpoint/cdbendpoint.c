...
static void check_parallel_cursor_errors(CdbDispatcherState *ds);
...
WaitEndpointReady(CdbDispatcherState* ds)
{
	...
	check_parallel_cursor_errors(ds);
}
...
check_parallel_retrieve_cursor(const char *cursorName, bool isWait)
{
	...
	ds = portal->queryDesc->estate->dispatcherState;
	retVal = cdbdisp_waitDispatchAckMessage(ds, ENDPOINT_FINISHED_ACK, isWait);
 ...
 */
void
check_parallel_cursor_errors(CdbDispatcherState *ds)
{
	/*
