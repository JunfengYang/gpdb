...
/*
 * PGExtTableEntry used in pg_exttable(). It stores each external
 * table entry in foreign table catalog.
 */
...
/*
 * PGExtTableEntriesContext used in pg_exttable() as user_fctx.
 */
 ...
 *
 * pg_exttable catalog gets removed because we use the FDW to implement
 * external table now.
 * But other extensions may still rely on the pg_exttable catalog.
 * So we create a view base on this UDF to extract pg_exttable catalog info.
 */
Datum pg_exttable(PG_FUNCTION_ARGS)
{
	...
	while (context->entryIdx < list_length(context->ftentries))
	{
		...
		/* options. Since our document not contains the OPTION caluse,
		   so we assume no external table options in used for now.
		   Except gpextprotocol.c. */
		nulls[5] = true;
